...
//    List<Map<String,Object>> getCountsByServiceNameForAppointment(Integer barber_id);
    ...
    @Query(nativeQuery = true, value =
            "WITH category_totals AS ( " +
                    ...
                    "), " +
                    "ranked_customers AS ( " +
                    "    SELECT s.category, c.id AS customer_id, u.first_name, u.last_name, " +
                    "    COUNT(aps.services_id) AS category_count, " +
                    ...
                    "    WHERE a.barber_id = :barberId " +
                    "    GROUP BY s.category, c.id, u.first_name, u.last_name, ct.total_count " +
                    ") " +
                    "SELECT category, customer_id, first_name, last_name, category_count " +
    ...
    @Query(nativeQuery = true, value =
            "WITH category_totals AS ( " +
                    ...
                    "    SELECT s.category, c.id AS customer_id, u.first_name, u.last_name, " +
                    "    COUNT(aps.services_id) AS category_count, " +
                    "    ROW_NUMBER() OVER (ORDER BY COUNT(aps.services_id) DESC) AS rn " +
                    "    FROM appointment a " +
                    "    JOIN appointment_services aps ON a.id = aps.appointment_id " +
                    "    JOIN services s ON aps.services_id = s.id " +
                    "    JOIN customer c ON a.customer_id = c.id " +
    ...
    @Query(nativeQuery = true, value =
            "WITH service_totals AS ( " +
                    ...
                    "), " +
                    "ranked_customers AS ( " +
                    "    SELECT s.category, s.service_name, u.first_name, u.last_name, " +
                    "    c.id AS customer_id, " +
                    "    COUNT(aps.services_id) AS service_count, " +
                    "    ROW_NUMBER() OVER (PARTITION BY s.service_name ORDER BY COUNT(aps.services_id) DESC) AS rn " +
                    "    FROM appointment a " +
                    "    JOIN appointment_services aps ON a.id = aps.appointment_id " +
                    "    JOIN services s ON aps.services_id = s.id " +
                    "    JOIN customer c ON a.customer_id = c.id " +
                    ...
                    "    WHERE a.barber_id = :barberId " +
                    "    GROUP BY s.category, s.service_name, c.id, u.first_name, u.last_name, st.total_count " +
                    ") " +
                    "SELECT category, customer_id, service_name, first_name, last_name, service_count " +
                    "FROM ranked_customers " +
                    "WHERE rn <= 5 " +
